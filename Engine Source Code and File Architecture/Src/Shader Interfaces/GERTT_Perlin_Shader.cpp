 ...
 *
 */
#include "GERTT_Perlin_Shader.h"

CGERTT_Perlin_Shader::CGERTT_Perlin_Shader()
{
...
CGERTT_Perlin_Shader::CGERTT_Perlin_Shader(const CGERTT_Perlin_Shader& source)
{
}

CGERTT_Perlin_Shader& CGERTT_Perlin_Shader::operator=(const CGERTT_Perlin_Shader& source)
{
	return *this;
}
...
bool CGERTT_Perlin_Shader::Initialize(ID3D11Device* pDevice, HWND hWnd)
{
	bool result;
	result = this->_initialize_shader(pDevice, hWnd, L"..\\Include\\Shaders\\RTT&Perlin Test.vs", 
		L"..\\Include\\Shaders\\RTT&Perlin Test.ps");
	if( !result )
		return false;
...
void CGERTT_Perlin_Shader::Shutdown()
{
	this->_shutdown();
}

bool CGERTT_Perlin_Shader::Render(ID3D11DeviceContext* pDeviceContext, int indexCount,
							  D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection,
							  ID3D11ShaderResourceView** pTextureArray)
{
	bool result;
	result = this->_set_shader_params(pDeviceContext, world, view, projection, pTextureArray);
	if( !result )
		return false;
...
bool CGERTT_Perlin_Shader::_initialize_shader(ID3D11Device* pDevice, HWND hWnd,
										  WCHAR* vs, WCHAR* ps)
{
	HRESULT result;
	...
	result = ::D3DX11CompileFromFile(vs, NULL, NULL,
		"RTT_Perlin_VS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
	...
	result = ::D3DX11CompileFromFile(ps, NULL, NULL,
		"RTT_Perlin_PS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0,
...
void CGERTT_Perlin_Shader::_shutdown()
{
...
void CGERTT_Perlin_Shader::_output_error_message(ID3D10Blob* pErrorMessage, HWND hWnd, WCHAR* pShaderFilename)
{
...
bool CGERTT_Perlin_Shader::_set_shader_params(ID3D11DeviceContext* pDeviceContext,
										  D3DXMATRIX world,							// World matrix
										  D3DXMATRIX view,							// View matrix
										  D3DXMATRIX projection,					// Projection matrix
										  ID3D11ShaderResourceView** pTextureArray)		// Texture source
{
	HRESULT						result;
...
void CGERTT_Perlin_Shader::_render_shader(ID3D11DeviceContext* pDeviceContext, int indexCount)
{
