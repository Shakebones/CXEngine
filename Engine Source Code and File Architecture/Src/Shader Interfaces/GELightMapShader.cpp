/*
 * File name - GELightMapShader.cpp
 * Author - CuiXue
...
CGELightMapShader::CGELightMapShader()
{
...
CGELightMapShader::CGELightMapShader(const CGELightMapShader& source)
{
}

CGELightMapShader& CGELightMapShader::operator=(const CGELightMapShader& source)
{
	return *this;
}
...
bool CGELightMapShader::Initialize(ID3D11Device* pDevice, HWND hWnd)
{
	bool result;

	result = this->_initialize_shader(pDevice, hWnd, L"../Include/Shaders/LightMap.vs", L"../Include/Shaders/LightMap.ps");
	if( !result )
		return false;
...
void CGELightMapShader::Shutdown()
{
	this->_shutdown();
}

bool CGELightMapShader::Render(ID3D11DeviceContext* pDeviceContext, int indexCount,
							   D3DXMATRIX world,
							   D3DXMATRIX view,
							   D3DXMATRIX proj,
...
void CGELightMapShader::_shutdown()
{
...
bool CGELightMapShader::_set_shader_params(ID3D11DeviceContext* pDeviceContext,
										   D3DXMATRIX world,
										   D3DXMATRIX view,
										   D3DXMATRIX proj,
										   ID3D11ShaderResourceView** pTextureArray)
...
bool CGELightMapShader::_output_error_message(ID3D10Blob* pErrorMessage, HWND hWnd, WCHAR* pShaderFilename)
{
...
void CGELightMapShader::_render_shader(ID3D11DeviceContext* pDeviceContext, int indexCount)
{
