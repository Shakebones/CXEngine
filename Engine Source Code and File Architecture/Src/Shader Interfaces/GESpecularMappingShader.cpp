/*
 * File name - GESpecularMappingShader.cpp
 * Author - CuiXue
...
CGESpecularMappingShader::CGESpecularMappingShader()
{
...
CGESpecularMappingShader::CGESpecularMappingShader(const CGESpecularMappingShader& source)
{
}

CGESpecularMappingShader& CGESpecularMappingShader::operator=(const CGESpecularMappingShader& source)
{
	return *this;
}
...
bool CGESpecularMappingShader::Initialize(ID3D11Device* pDevice, HWND hWnd)
{
	bool result;

	result = this->_initialize_shader(pDevice, hWnd, L"../Include/Shaders/SpecularMapping.vs",
													 L"../Include/Shaders/SpecularMapping.ps");
	if( !result )
		return false;
...
void CGESpecularMappingShader::Shutdown()
{
	this->_shutdown();
}

bool CGESpecularMappingShader::Render(ID3D11DeviceContext* pDeviceContext, int indexCount, D3DXMATRIX world,
		D3DXMATRIX view, D3DXMATRIX proj, ID3D11ShaderResourceView** ppTextureArray, D3DXVECTOR3 lightDir,
		D3DXVECTOR3 cameraPos, D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor, D3DXVECTOR4 specularColor, 
		float specularPower)
{
	bool result;

	result = this->_set_shader_params(pDeviceContext, world, view, proj, ppTextureArray, lightDir, cameraPos,
...
bool CGESpecularMappingShader::_initialize_shader(ID3D11Device* pDevice, HWND hWnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "SpecularMappingVS", "vs_5_0", 
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "SpecularMappingPS", "ps_5_0",
	...
	if( FAILED(result) )
	{
		...
		{
			MessageBox(hWnd, psFilename, L"Missing Shadre File", MB_OK);
		}
...
void CGESpecularMappingShader::_shutdown()
{
...
void CGESpecularMappingShader::_output_error_message(ID3D10Blob* pErrorMessage, HWND hWnd, WCHAR* shaderFilename)
{
	char* pCompileError;
...
bool CGESpecularMappingShader::_set_shader_params(ID3D11DeviceContext* pDeviceContext, D3DXMATRIX world, 
												  D3DXMATRIX view, D3DXMATRIX proj,
												  ID3D11ShaderResourceView** ppTextureArray, 
												  D3DXVECTOR3 lightDir, D3DXVECTOR3 cameraPos, 
												  ...
												  D3DXVECTOR4 specularColor, float specularPower)
{
	...
	pDeviceContext->PSSetShaderResources(0, 3, ppTextureArray);
...
void CGESpecularMappingShader::_render_shader(ID3D11DeviceContext* pDeviceContext, int indexCount)
{
