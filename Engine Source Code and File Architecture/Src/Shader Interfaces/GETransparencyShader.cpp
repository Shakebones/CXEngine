...
CGETransparencyShader::CGETransparencyShader()
{
	this->m_pVertexShader = 0;
	this->m_pPixelShader = 0;
	this->m_pInputLayout = 0;
	this->m_pMatrixBuffer = 0;
	this->m_pTransparency = 0;
	this->m_pSamplerState = 0;
}

CGETransparencyShader::CGETransparencyShader(const CGETransparencyShader& source)
{
}

CGETransparencyShader& CGETransparencyShader::operator=(const CGETransparencyShader& source)
{
	return *this;
}
...
bool CGETransparencyShader::Initialize(ID3D11Device* pDevice, HWND hWnd)
{
	bool result;

	result = this->_initialize_shader(pDevice, hWnd,
		L"../Include/Shaders/TexTrans.vs", L"../Include/Shaders/TexTrans.ps");
	if( !result )
		return false;
...
void CGETransparencyShader::Shutdown()
{
	this->_shutdown();
}

bool CGETransparencyShader::Render(ID3D11DeviceContext* pDeviceContext, int indexCount,
							  ...
							  float transparency)
{
	...
	result = this->_set_shader_params(pDeviceContext, world, view, proj, pTexture, transparency);
	if( !result )
		return false;
...
bool CGETransparencyShader::_initialize_shader(ID3D11Device* pDevice, HWND hWnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
	...
	D3D11_BUFFER_DESC transparencyBufferDesc;
	...
	result = D3DX11CompileFromFile(vsFilename, NULL, NULL, "TexTransVS", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS,
	...
	result = D3DX11CompileFromFile(psFilename, NULL, NULL, "TexTransPS", "ps_5_0", D3D10_SHADER_ENABLE_STRICTNESS,
	...
	transparencyBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
	transparencyBufferDesc.ByteWidth = sizeof(TransparencyBufferType);
	transparencyBufferDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	transparencyBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	transparencyBufferDesc.MiscFlags = 0;
	transparencyBufferDesc.StructureByteStride = 0;

	result = pDevice->CreateBuffer(&transparencyBufferDesc, NULL, &this->m_pTransparency);
	if( FAILED(result) )
		return false;
...
void CGETransparencyShader::_shutdown()
{
	::ReleaseWithoutDel<ID3D11SamplerState*>(this->m_pSamplerState);
	::ReleaseWithoutDel<ID3D11Buffer*>(this->m_pTransparency);
...
void CGETransparencyShader::_output_error_message(ID3D10Blob* pErrorMessage, HWND hWnd, WCHAR* pShaderFilename)
{
	char* pCompileError;
...
bool CGETransparencyShader::_set_shader_params(ID3D11DeviceContext* pDeviceContext,
										  D3DXMATRIX world,
										  D3DXMATRIX view,
										  D3DXMATRIX proj,
										  ID3D11ShaderResourceView* pTexture,
										  float transparency)
{
	...
	TransparencyBufferType* pData_transparency;
	...
	result = pDeviceContext->Map(this->m_pTransparency, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResources);
	if( FAILED(result) )
		return false;

	pData_transparency = (TransparencyBufferType*)mappedResources.pData;

	pData_transparency->transparency = transparency;

	pDeviceContext->Unmap(this->m_pTransparency, 0);

	bufferNumber = 0;

	pDeviceContext->PSSetConstantBuffers(bufferNumber, 1, &this->m_pTransparency);

	return true;
}

void CGETransparencyShader::_render_shader(ID3D11DeviceContext* pDeviceContext, int indexCount)
{
